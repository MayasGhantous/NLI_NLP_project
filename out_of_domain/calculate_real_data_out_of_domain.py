...
UNIGRAM_LOCATOIN = 'out_of_domain\\calculated_data\\Unigrams'
ERROR_HELPER_LOCATION = "out_of_domain\\calculated_data\\error_count2"
FUNCTION_WORDS_ARRAY_LOCATION = 'out_of_domain\\calculated_data\\function_words.npy'

FUNCTION_WORDS_ARRAY_LOCATION = 'out_of_domain\\calculated_data\\function_words_features'

SENTENCE_LENGTH_LOCATION = 'out_of_domain\\average_sentence_lengths.npy'

TRIGRAM_LOCATION = 'out_of_domain\\calculated_data\\charracter_trigrams'  
GRAMMAR_ERRORS_LOCATION = 'out_of_domain\\calculated_data\\grammer_errors'
POS_TRIGRAM_LOCATION = 'out_of_domain\\top_pos_trigrams.npy'
GET_ALL_FIELS = False

EDIT_DISTEANCE_LOACTION = 'out_of_domain\\calculated_data\\edit_distance'
RULES_LOCATION = 'out_of_domain\\calculated_data\\rules.npy'
TOP_ERROR_LOCATION = 'calculated_data\\error_count2'
...
def get_top_errors(top=400):
    ...
    sub_errors = Counter()
    for error_file in os.listdir(TOP_ERROR_LOCATION):
        if error_file.startswith('all_errors'):
            current = np.load(os.path.join(TOP_ERROR_LOCATION, error_file), allow_pickle=True).item()
...
def get_edit_distance():
    ...
    for edit_distance_file in tqdm.tqdm(os.listdir(EDIT_DISTEANCE_LOACTION)):
