...
def extract_pos_trigram_features(dic, top_n=300):
    ...
    top_trigrams = np.load("top_300_pos.npy", allow_pickle=True)
    ...
    for key in tqdm.tqdm(dic.keys()):
        for content in dic[key]:
            ...
            for trigram in top_trigrams:
                frequency = trigram_counts[(trigram[0],trigram[1],trigram[2])] / total_trigrams if total_trigrams > 0 else 0
...
files = None
files = read_files_from_directory('non_europe_data')
...
if DO_WE_NEED_TO_EXTRACT_TRIGRAMS:
    ...
    for i in list_of_numbers:
        ...
        files1 = read_files_from_directory('non_europe_data')
